...
from orcas_v01_manutencao import renderizar_pagina_manutencao
...
def exibir_admin(df, supabase, ID_USUARIO_LOGADO, ir_para_o_topo):
  """Hub principal da Tela Admin com cabeçalho padrão e navegação via radio buttons
  ...
  with col_titulo:
    st.markdown(
        '<div class="titulo-tela" style="margin-top:0px;">Administração:'
        f" {projeto_ativo}</div>",
  ...
  with col_ajuda:
    ...
    if st.button(
        ...
        key="btn_ajuda_admin_main",
    ):
      st.session_state["exibir_ajuda_admin"] = not st.session_state.get(
          "exibir_ajuda_admin", False
      )
      st.rerun()
  ...
  if opcao_admin == "🛠️ Manutenção em Lote":
    renderizar_pagina_manutencao(supabase, ID_USUARIO_LOGADO, projeto_ativo)
