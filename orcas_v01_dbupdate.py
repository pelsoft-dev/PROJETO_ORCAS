...
def renderizar_dbupdate(df, supabase, ID_USUARIO_LOGADO, ir_para_o_topo):
    """
    Sub-rotina da Tela Admin - Edição direta em massa no banco de dados (Layout Excel).
    """
    ...
    with col_titulo:
        st.markdown(
            f'<div class="titulo-tela" style="margin-top:0px;">Edição Direta (DB Update): {st.session_state.get("projeto_ativo", "")}</div>',
    ...
    colunas_existentes = [c for c in colunas_ordem if c in df_admin.columns]
    
    if colunas_existentes:
        ordem_selecionada = st.selectbox("Ordenar visualização por:", colunas_existentes, index=0)
