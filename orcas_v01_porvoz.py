...
def processar_texto_groq(
  ...
  prompt = f"""
    ...
    1. INTENÇÃO:
       ...
       - "ALTERAR": Modificar valor, nome, descrição ou data de uma conta existente (EX: "Mudar a descrição de X para Y").
       - "EXCLUIR": APENAS quando o usuário usar termos explícitos como "deletar", "excluir", "apagar", "remover". NUNCA interprete solicitações de "mudar" ou "alterar" como EXCLUIR.
       - "CONSULTAR": Perguntas sobre saldos, totais ou resumos.

    2. DATA DE VENCIMENTO (data_vencimento):
    ...
    3. MÊS DE REFERÊNCIA (mes_referencia):
       - Se o usuário mencionou um mês específico (ex: "em julho", "de junho"), retorne o mês numérico (1 a 12). Caso contrário, retorne null.
    ...
    {{
      ...
      "intencao": "PROJETAR" | "REALIZAR" | "ALTERAR" | "EXCLUIR" | "CONSULTAR",
...
def buscar_planejamento_existente(
  ...
  try:
    ...
    # Filtragem estrita por mês se especificado pelo usuário (ex: Julho -> mês 7)
...
def executar_acao_no_supabase(supabase, usuario_id, dados):
  """Executa inclusão, alteração, baixa ou exclusão no Supabase."""
      ...
      if hash_atual != st.session_state.hash_ultimo_audio:
        with st.spinner('🤖 ORCAS está processando o áudio...'):
          try:
            ...
            intencao = dados.get('intencao')
            incluir_realizados = intencao in ['EXCLUIR', 'ALTERAR']
            ...
            if item_existente:
              ...
              # Usa a data cadastrada no banco caso o usuário não tenha definido uma data nova explicitamente
              ...
              if intencao == 'EXCLUIR':
                dados['mensagem_orcas'] = (
                    ...
                    ' realmente **EXCLUIR** este lançamento?'
                )
  ...
  # ETAPA 2: CONFIRMAÇÃO E EDIÇÃO DOS DADOS PELO USUÁRIO
    ...
    with st.container(border=True):
      ...
      with st.form('form_confirmacao_orcas'):
        ...
        acoes_opcoes = ['REALIZAR', 'PROJETAR', 'ALTERAR', 'EXCLUIR']
        ...
        with col_2:
          ...
          # Trata e exibe o campo de Data de Vencimento Editável
          try:
          ...
          nova_data_venc = st.date_input(
              'Data de Vencimento', value=dt_val, format='DD/MM/YYYY'
          )
