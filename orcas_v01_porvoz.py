  ...
  if projeto_id:
    try:
      ...
      if res and res.data:
        ...
        if d_ini:
          dt_ini_valida = datetime.strptime(
              str(d_ini)[:10], "%Y-%m-%d"
          ).date()
        if d_fim:
          dt_fim_valida = datetime.strptime(
              str(d_fim)[:10], "%Y-%m-%d"
          ).date()
...
def buscar_lancamento_no_banco(supabase, usuario_id, projeto_id, descricao):
  ...
  desc_limpa = re.sub(
      r"\d+\s*de\s*\d+", "", desc_limpa, flags=re.IGNORECASE
  ).strip()
  ...
  # TELA 2: CONFIRMAÇÃO
    ...
    if cartao_detectado:
      ...
      match_opt = next(
          (
              opt
              for opt in opcoes_cartoes
              if opt.upper() == cartao_clean.upper()
          ),
          None,
      )
      if match_opt:
    ...
    with st.form("form_confirmacao_voz"):
      ...
      if sub_salvar:
        ...
        if intencao_selecionada == "PROJETAR":
          ...
          # ESTRUTURA REAJUSTADA PARA ENCAIXAR PERFEITAMENTE NO CONCILIAÇÃO
          dados_finais = {
              ...
              # PASSAGEM EXATA DAS CHAVES DE CARTÃO
...
def exibir_modal_voz_orcas(supabase, id_usuario, planos_disponiveis=None):
  ...
  if not st.session_state.exibir_modal_voz and not st.session_state.get(
      "abrir_modal_orcas", False
  ):
    return
