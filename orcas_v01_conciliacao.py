...
def exibir_conciliacao(df, supabase, ID_USUARIO_LOGADO, format_moeda, parse_moeda):
    """
    Sub-rotina da Tela Conciliação - Solução Estrita de Filtragem de Parciais e Ocultação de Cartões.
    """
    ...
    if not df_c.empty:
        ...
        # 🟢 REGRA DE OURO: REMOVE TOTALMENTE CARTÕES ($CCP e $CCL) DA CONCILIAÇÃO
        if 'tipo_cc' in df_c.columns:
            df_c = df_c[~df_c['tipo_cc'].isin(['$CCP', '$CCL'])].copy()

        # 🟢 REGRA CRÍTICA: SEPARA TOTALMENTE OS REGISTROS DE PARCIAL DA LISTA PRINCIPAL
        # 1. Guarda o histórico de parciais para somar no V.Real
        ...
        if not df_final_concilia.empty:
            # CABEÇALHO
