...
# --- TELA: PROJETAR ---
    ...
    if btn_col2.button("Excluir", use_container_width=True):
        if not desc: 
            ...
            st.rerun()
            # --- TELA: CONCILIAÇÃO (REGRAS REFINADAS) ---
    ...
    # Definição de datas para os filtros (Solicitação 3)
    hoje = datetime.now().date()
    inicio_mes = hoje.replace(day=1)
    limite_retroativo = hoje - timedelta(days=3)

    # 1. Filtro de segurança: Não listar datas futuras
    df_c = df.copy()
    df_c['dt_obj'] = pd.to_datetime(df_c['data']).dt.date
    
    # 2. Aplicar lógica de exibição:
    # - Planejados até hoje
    # - Realizados apenas dos últimos 3 dias
    df_f = df_c[
        (df_c['dt_obj'] <= hoje) & 
        (
            (df_c['status'] == 'Planejado') | 
            ((df_c['status'] == 'Realizado') & (df_c['dt_obj'] >= limite_retroativo))
        )
    ].copy()

    # 3. Ordenação (Solicitação 3): 
    # Parciais do MÊS ATUAL no topo, depois o restante por data decrescente
    parciais_topo = df_f[(df_f['permite_parcial'] == True) & (df_f['dt_obj'] >= inicio_mes)]
    ...
    # Cabeçalho da Tabela de Conciliação
    ...
    for _, row in df_final_concilia.iterrows():
        ...
        data_fmt = row['dt_obj'].strftime('%d/%m/%y')
        ...
        # Lógica de exibição de valores e inputs
        if row['permite_parcial']:
            c4.write(format_moeda(row['valor_real']))
            v_parc_in = c5.text_input("R$", key=f"inp_parc_{row['id']}", value="0,00")
            if c6.button("Confirmar", key=f"btn_parc_{row['id']}"):
                v_digitado = parse_moeda(v_parc_in)
                if v_digitado > 0:
                    # Insere o registro filho
                    supabase.table("lancamentos").insert({
                        "projeto_id": row['projeto_id'], 
                        "usuario_id": uid, 
                        "data": hoje.strftime('%Y-%m-%d'),
                        "descricao": f"Pcl: {row['descricao']}", 
                        "valor_real": v_digitado, 
                        "status": "Realizado", 
                        "tipo": row['tipo'], 
                        "id_pai": row['id']
                    }).execute()
                    # Atualiza o acumulado no pai
                    supabase.table("lancamentos").update({
                        "valor_real": row['valor_real'] + v_digitado
            ...
            if row['status'] == 'Realizado':
                ...
                c4.write("-")
                v_norm_in = c5.text_input("R$", key=f"inp_norm_{row['id']}", value=format_moeda(row['valor_plan']))
                if c6.button("OK", key=f"btn_norm_{row['id']}"):
                    supabase.table("lancamentos").update({
