...
st.set_page_config(page_title="Orcas Pro", layout="wide", initial_sidebar_state="collapsed")

st.markdown("""
    <style>
    /* Esconder botões nativos do Streamlit */
    #MainMenu {visibility: hidden;}
    header {visibility: hidden;}
    footer {visibility: hidden;}
    .stDeployButton {display:none;}
    
    /* Fixar o Header no topo de forma limpa */
    .fixed-header {
        position: fixed;
        top: 0;
        left: 0;
    ...
    /* Ajuste para campos de entrada de texto no header */
...
if escolha == "⚙️ Gestão":
    ...
    with st.container():
        ...
        with col2:
            ...
            if sum([bool(dia_mes), bool(dia_sem), (dia_esp is not None)]) > 1:
                st.error("⚠️ ERRO: Escolha apenas UM critério.")
        with col3:
            d_ini = st.date_input("Início", value=st.session_state.d_mestre_ini, format="DD/MM/YYYY")
            d_fim = st.date_input("Término", value=st.session_state.d_mestre_fim, format="DD/MM/YYYY")
        ...
        if c_b1.button("Incluir"):
            ...
            while curr <= d_fim:
                match = (dia_esp and curr == dia_esp) or (dia_mes == "*") or (dia_mes and str(curr.day) == dia_mes) or (dia_sem != "" and curr.strftime('%A') == dias_map[dia_sem])
                if match:
    ...
    if not df_c.empty:
        ...
        for i, row in df_c.iterrows():
            ...
            # Salvar automaticamente se mudar valor
