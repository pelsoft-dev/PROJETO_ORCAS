...
# --- TELA: PROJETAR ---
# --- TELA: PROJETAR ---
# --- TELA: PROJETAR (VERSÃO EVOLUÍDA) ---
# --- TELA: PROJETAR (CORRIGIDA E COMPLETA) ---
# --- TELA: PROJETAR (VERSÃO FINAL BLINDADA) ---
# --- TELA: PROJETAR (VERSÃO PROJEÇÃO AVANÇADA) ---
    ...
    # --- EXPANDER 1: RECORRÊNCIA (ESTRUTURA JÁ CONHECIDA) ---
    ...
    # --- EXPANDER 2: PROJEÇÃO AVANÇADA (NOVO LAYOUT) ---
    ...
    if btn_col1.button("Incluir", use_container_width=True):
        ...
        while curr <= limite_loop:
            # Lógica de match (dd/mm, dia fixo ou *)
            match_dm = False
            if "/" in d_m:
                try:
            ...
            if (d_e is None or curr == d_e) and match_dm and (d_s == "" or curr.weekday() == d_map[d_s]):
                ...
                # Regra de Complemento (Apenas final conforme solicitado)
                ...
                lista_bulk.append({
                    ...
                    "data_vencimento": dt_f.strftime('%Y-%m-%d'), # Blindagem contra erro
                ...
                gerados += 1
                # Incremento de valor se for correção mensal
...
# --- TELA: CONCILIAÇÃO (VERSÃO AVANÇADA) ---
    ...
    # Filtramos para não mostrar realizados com mais de 3 dias
    data_limite_retroativa = (datetime.now() - timedelta(days=3)).strftime('%Y-%m-%d')
    
    # Pegamos tudo que é Planejado OU Realizado nos últimos 3 dias
    df_conciliacao = df[
        (df['status'] == 'Planejado') | 
        ((df['status'] == 'Realizado') & (df['data'] >= data_limite_retroativa))
    ].copy()

    # 2. SEPARAÇÃO POR CATEGORIAS (Para o Topo)
    ...
    for _, row in parciais.iterrows():
        ...
        # Lógica de Data (mmm/aa ou aaaa) conforme Projeção Avançada
        data_dt = datetime.strptime(row['data'], '%Y-%m-%d')
        label_data = data_dt.strftime('%b/%y').upper() # Ex: MAR/26
        ...
        if c6.button("Confirmar", key=f"btn_{row['id']}"):
            ...
            if valor_digitado > 0:
                # Ações: 1. Criar o registro 'Filho' Realizado / 2. Atualizar o 'Pai'
                novo_realizado_total = row['valor_real'] + valor_digitado
                
                # Registro Filho (para o extrato e gráficos)
                ...
                # Atualiza o Pai (apenas para controle da barra de progresso na tela)
    ...
    for _, row in normais.iterrows():
        # ... (Lógica similar para os lançamentos dia a dia) ...
        # Se status == 'Realizado', mostra o ícone de "OK" verde
        # Se status == 'Planejado', mostra o botão "Confirmar"
        pass

    # 5. RECURSO EXTRA: REALIZAR SEM PLANEJAMENTO (RODAPÉ)
    st.markdown("---")
