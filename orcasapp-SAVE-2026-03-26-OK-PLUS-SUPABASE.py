...
# --- 4. ESTADO E DADOS ---
# Forçamos o uid a vir do session_state, mas vamos garantir que ele exista
...
# Verificação de segurança (calcula os dias na hora, sem salvar em variável global)
if uid:
    dias_para_seguranca = (venc_dt_objeto - datetime.now().date()).days
    security.verificar_bloqueio_v01(uid, dias_para_seguranca)
...
with st.sidebar:
    ...
    # Exibição da data de validade (Opção A) - Mata o conflito do número "365"
...
# Redirecionamento forçado para Gestão se não houver projeto
...
# --- TELA: CONCILIAÇÃO (BLINDAGEM TOTAL ANTI-365) ---
    ...
    for _, row in df_final_concilia.iterrows():
        # Lógica de cor automática se ultrapassar
        cor_txt = "red" if row['valor_real'] > row['valor_plan'] else "black"
        
        # Altura compactada aprovada
        ...
        if row['permite_parcial']:
            ...
            v_parc_in = c5.text_input("", key=f"p_{id_item}", value="0,00", label_visibility="collapsed")
            
            if c6.button("Confirmar", key=f"btn_p_{id_item}"):
                v_dig = parse_moeda(v_parc_in)
                if v_dig > 0:
                    # --- BUSCA DE SEGURANÇA MÁXIMA ---
                    # Buscamos o ID direto pelo e-mail, ignorando qualquer 'st.session_state.user_id'
                    user_valid = supabase.table("usuarios").select("id").eq("email", st.session_state.usuario).execute()
                    safe_uuid = user_valid.data[0]['id']
                    
                    # 1. Salva o log da parcial
                    supabase.table("lancamentos").insert({
                        "projeto_id": str(st.session_state.projeto_ativo), 
                        "usuario_id": str(safe_uuid), 
                        "data": hoje_c.strftime('%Y-%m-%d'),
                        "descricao": f"Pcl: {row['descricao']}", 
                        "valor_real": v_dig, 
                        "status": "Realizado", 
                        "tipo": row['tipo'], 
                        "id_pai": id_item 
                    ...
                    # 2. Atualiza o valor acumulado (SOMA)
                    novo_total = float(row['valor_real'] + v_dig)
                    supabase.table("lancamentos").update({"valor_real": novo_total}).eq("id", id_item).execute()
                    st.rerun()
        else:
            if row['status'] == 'Realizado':
                ...
                v_norm_in = c4.text_input("", key=f"n_{id_item}", value=format_moeda(row['valor_plan']), label_visibility="collapsed")
                if c6.button("Confirmar", key=f"btn_n_{id_item}"):
                    user_valid = supabase.table("usuarios").select("id").eq("email", st.session_state.usuario).execute()
                    safe_uuid = user_valid.data[0]['id']
