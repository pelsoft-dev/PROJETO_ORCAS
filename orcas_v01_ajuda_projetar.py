...
def renderizar_ajuda_projetar():
    """
    Renderiza o box de ajuda da tela de Projetar com a estilização correta.
    ...
    st.markdown(
        ...
        <div style="background-color: #007ba7; padding: 15px; border-radius: 5px; color: white; font-family: sans-serif; margin-bottom: 20px; position: relative;">
            <div style="font-size: 14px; text-align: justify; line-height: 1.4;">
                PROJETAR Se for sua primeira vez aqui no ORCAS, siga esse caminho: (1) digite um nome para seu Plano no campo 02 e dê <enter>. (2) Se você quiser partir de           Esse Plano será criado contendo <b>24 meses</b> (padrão) iniciando a partir de hoje. Se você quiser 
    ...
    # <div style="font-weight: bold; font-size: 16px; margin-bottom: 8px;">AJUDA – PROJETAR</div>

    if st.button("❌ Fechar Guia de Ajuda", key="btn_fechar_ajuda_projetar"):
        st.session_state["exibir_ajuda_projetar"] = False
        st.rerun()