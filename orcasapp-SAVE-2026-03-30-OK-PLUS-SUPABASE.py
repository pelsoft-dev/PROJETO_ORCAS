...
# --- TELA: LANÇAMENTOS (INTEGRAÇÃO COMPLETA) ---
    ...
    if d_ini_db and d_fim_db:
        ...
        for mes_str in meses_periodo:
            ...
            # ACERTO: Soma apenas o maior entre Planejado e Realizado Acumulado para evitar duplicidade
            entradas_mes = df_mes[df_mes['tipo'] == 'Entrada'].apply(
                lambda x: max(x['valor_plan'], df_mes[(df_mes['descricao'] == x['descricao']) & (df_mes['valor_plan'] == 0)]['parcial_real'].sum()) if x['valor_plan'] > 0 else 0, axis=1
            ).sum()
...
# --- TELA: CONCILIAÇÃO ---
    ...
    for _, row in df_final_concilia.iterrows():
        ...
        if row['permite_parcial']:
            ...
            # EXIBIÇÃO: Valor Real mostra o acumulado atualizado
            c4.markdown(f"<span style='color:{cor_txt}'>{format_moeda(v_acumulado_desc)}</span>", unsafe_allow_html=True)
            
            # ACERTO DEFINITIVO: Uso de sufixo na key para forçar reset do widget após o clique
            # Criamos uma chave no session_state para controlar a versão do widget se ela não existir
            v_key = f"v_p_{row['id']}"
            if v_key not in st.session_state:
                st.session_state[v_key] = 0
            
            # A key muda toda vez que o botão é clicado, limpando o campo visualmente
            ...
            if c6.button("Confirmar", key=f"btn_p_{row['id']}"):
                ...
                if v_dig > 0:
                    ...
                    # ACERTO: Muda a versão da key para o widget "nascer" limpo no próximo ciclo
...
# --- TELA: GESTÃO (REGRAS DE DATAS E SALDO) ---
    ...
    # DEFINIÇÃO DA VARIÁVEL HOJE PARA EVITAR NAMEERROR
    hoje = datetime.now().date()
    # CAPTURA DO UID DO SESSION STATE
    ...
    # ACERTO CRÍTICO: Garante a permanência absoluta na tela de Gestão
    if plano_sel != "" and plano_sel != st.session_state.get('projeto_ativo'):
        st.session_state.projeto_ativo = plano_sel
        # Forçamos a variável 'escolha' no state ANTES do rerun para o roteador não se perder
    ...
    # ACERTO: Só mostra os detalhes e o cálculo de valor se houver um nome definido ou plano carregado
    if nome_plano_input and nome_plano_input.strip() != "":
        col_l2_1, col_l2_2 = st.columns(2)
        
        # ACERTO: Data de início = Hoje / Data de Término = 24 meses (730 dias) após hoje
        ...
        # Cálculo de meses do Período do Plano
        ...
        # ACERTO: O cálculo da assinatura e o card azul só aparecem se houver plano ativo (Zera o erro do R$ xx,xx residual)
        ...
        if btn_col1.button("Salvar alterações ou Criar o novo Plano", use_container_width=True):
            ...
            # ACERTO: Força permanência na tela de Gestão após o salvamento
            st.session_state.escolha = "⚙️ Gestão"
            st.rerun()
        ...
        # Mensagem informativa: Garante que o valor da assinatura não apareça se o input estiver vazio
        st.info("Por favor, selecione um plano existente ou digite um nome para iniciar a configuração.")

    # LISTAGEM COMPLETA DAS OBSERVAÇÕES NO RODAPÉ
    st.markdown("""
